...
const ForecastChart = () => {
  ...
  const loadForecastInsights = async () => {
    try {
      ...
      const forecastInsights = await apiService.get(`/api/forecast/insights?metric=${metric}&model=${selectedModel}`);
      setInsights(forecastInsights);
    } catch (err) {
      console.error('Error loading forecast insights:', err);
    } finally {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="mt-6">
        <ForecastInsights insights={insights} loading={insightsLoading} />
      </div>
    </div>
  );
};
